...
module.exports.run = async function({ api, args, event, Users, permssion, Currencies }) {
  ...
  if(!args[0]){
    const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/chientruong?senderID=${senderID}`)
    ...
    const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/dangky?uid=${senderID}&name=${encodeURI(lmao)}`)
    if( `${res.data.data}` == "false"){
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    } else {
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    ...
    const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/`)
    if( `${res.data.data}` == "false"){
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    } else {
...
module.exports.handleReply = async function({
  ...
  switch (handleReply.type) {
    case "choosee": {
      switch (event.body) {
        case '1': {
        const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/listitem?type=item`)
             if( `${res.data.data}` == "false"){
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    } else {
 return api.sendMessage(`${res.data.msg}[ Reply ]`, threadID, (error, info) => {
          ...
          case"2":{
             const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/listitem?type=xe`)
             if( `${res.data.data}` == "false"){
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    } else {
 return api.sendMessage(`${res.data.msg}[ Reply ]`, threadID, (error, info) => {
...
const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/buy?senderID=${senderID}&choose=${event.body}`)
      if( `${res.data.data}` == "false"){
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    } else {
          if (moneyUser < money) {
...
const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/xe?senderID=${senderID}&choose=${event.body}`)
      if( `${res.data.data}` == "false"){
      return api.sendMessage(`${res.data.msg}`,threadID, messageID)
    } else {
        if (moneyUser < money) {
   ...
   switch (event.body) {
        case event.body: {
          const res = await axios.get(`https://ZWAR-1.araxy-ox-qwqbqiws.repl.co/doikhu?senderID=${senderID}&choose=${event.body}`)
          if( `${res.data.data}` == false){
            return api.sendMessage(`${res.data.msg}`, threadID, messageID)
          } else {
             return api.sendMessage(`${res.data.msg}`, threadID, messageID)
