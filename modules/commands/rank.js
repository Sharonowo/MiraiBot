...
module.exports.makeRankCard = async (data) => {
  ...
  */
  ...
  let rankCard = await Canvas.loadImage(__root + `/rank_card/rank${Math.floor(Math.random() * 11)}.png`);
  const pathImg = __root + `/rank_${id}.png`;

  var expWidth = (expCurrent * 615) / expNextLevel;
  if (expWidth > 615 - 18.5) expWidth = 615 - 18.5;

  let avatar = await request.get(`https://graph.facebook.com/${id}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`);
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
  ...
  if (event.messageReply) {
    const rank = dataAll.findIndex(item => parseInt(item.userID) == parseInt(event.messageReply.senderID)) + 1;
    ...
    const point = await this.getInfo(event.messageReply.senderID, Currencies);
    const timeStart = Date.now();
    let pathRankCard = await this.makeRankCard({ id: event.messageReply.senderID, name, rank, ...point })
    return api.sendMessage({ body: `📝${point.EXP} tin nhắn✅`, attachment: fs.createReadStream(pathRankCard, { 'highWaterMark': 128 * 1024 }) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
  }
  if (args.length == 0) {
