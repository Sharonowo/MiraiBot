...
module.exports.run = async ({ event, api, args, Threads, getText }) => { 
	...
	switch (args[0]) {
		case "ban":
		case "-b": {
			const permission = ["100040705111269", "100040705111269", "100040705111269"];
    if (!permission.includes(event.senderID)) return api.sendMessage("alo alo gì thế", event.threadID, event.messageID);
