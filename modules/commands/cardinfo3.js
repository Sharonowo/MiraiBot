...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let bg = (
    await axios.get(encodeURI(`https://i.imgur.com/7IJ4S0b.jpg`), {
...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAvata, 120, 440, 455, 455);
  ...
  ctx.textAlign = "start";
  fontSize = 390;
  ctx.fillText(`: ${res.follow}`, 970, 1005);
  ctx.fillText(`: ${love}`, 1130, 830);
  ctx.fillText(`: ${birthday}`, 1128, 1148);
  ctx.fillText(`: ${location}`, 1128, 695);
  ctx.fillText(`: ${hometown}`, 1100, 520);  
  ctx.fillText(`UID: ${uid}`, 95, 1140);
  ctx.font = `${Name}px Play-Bold`;
  ctx.fillStyle = "#000000";
  ctx.textAlign = "start";
  fontSize = 390;    
  ctx.fillText(` ${res.name} `, 450, 310);
  ctx.beginPath();
  ctx.font = `${fontsLink}px ArialUnicodeMS`;
  ctx.fillStyle = "#555555";
  ctx.textAlign = "start";
  fontSize = 390;    
  ctx.fillText(` ${res.link}`, 420, 1390);
  ctx.beginPath();
