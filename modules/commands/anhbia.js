...
module.exports.run = async function ({ api, event, args, Users }) {
    ...
    if(!args[0]) return api.sendMessage({ body: `Vui lòng nhập theo định dạng: ID/ + Chữ ký + Chữ nền + Màu`              
    }, event.threadID)
...
let {data} = ( await axios.get('https://sumibot-api.herokuapp.com/list'))
...
setTimeout(() => {
    api.unsendMessage(info.messageID) } , 8000),event.messageID);
         ...
         let GetImage = (await axios.get(encodeURI(`https://sumibot-api.herokuapp.com/biaAnime?id=${id}&text1=${chu_ky}&text2=${chu_nen}&color=${maunen}`), {responseType: "arraybuffer"})).data;  
