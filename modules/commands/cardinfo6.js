...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let bg = (
    await axios.get(encodeURI(`https://i.imgur.com/k0d0wiu.jpg`), {
...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAvata, 101, 249, 287, 287);
  ...
  ctx.textAlign = "start";
  fontSize = 390;
  ctx.fillText(` ${res.follow} Người theo dõi`, 560, 527);
  ctx.fillText(` ${love}`, 679, 453);
  ctx.fillText(` ${birthday}`, 675, 606);
  ctx.fillText(` ${location}`, 649, 378);
  ctx.fillText(` ${hometown}`, 637, 293);
  ctx.fillText(` ${uid}`, 90, 613);
  ctx.font = `${Name}px VNARABIA`;
  ctx.fillStyle = "#FFCC66";
  ctx.textAlign = "start";
  fontSize = 390;    
  ctx.fillText(` ${res.name} `, 325, 180);
  ctx.beginPath();
  ctx.font = `${fontsLink}px ArialUnicodeMS`;
  ctx.fillStyle = "#FF0000";
  ctx.textAlign = "start";
  fontSize = 390;    
  ctx.fillText(` ${res.link}`, 200, 725);
  ctx.beginPath();
