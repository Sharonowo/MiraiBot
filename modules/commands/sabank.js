...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    switch(args[0]) {
      ...
      case '-c': {
        ...
        case 'password':
        case 'pw': {
            if(checkBank.status == false) return api.sendMessage('Bạn chưa có tài khoản SACOMBANK!', threadID, messageID);
            var type = args[1];
            switch(type) {
                case 'get': {
