...
module.exports.run = async function({ api, event, Users, Threads }) {
    ...
    switch (logMessageType) {
        case "log:thread-color":
            {
                return api.sendMessage(`»[ THÔNG BÁO ]«\nNhóm đã thay đổi chủ đề thành: ${event.logMessageData.magic_word || event.logMessageData.accessibility_label }\nEmoij: ${event.logMessageData.theme_emoji || "Không có emoji"}`, threadID)
            }
        case "log:magic-words":
            {
                return api.sendMessage(`»[ THÔNG BÁO ]«\nTheme ${event.logMessageData.magic_word} đã thêm hiệu ứng: ${event.logMessageData.theme_name}\nEmoij: ${event.logMessageData.emoji_effect || "Không có emoji"}\nTổng ${event.logMessageData.new_magic_word_count} hiệu ứng từ ngữ được thêm vào`, threadID)
            }
